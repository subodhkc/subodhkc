...
import { StudentDetailClient } from '@/components/app/school-pickup/StudentDetailClient'
...
export default async function StudentDetailPage({
  ...
  params: Promise<{ orgSlug: string; siteSlug: string; studentId: string }>
}) {
  const { orgSlug, siteSlug, studentId } = await params
  const user = await getAuthenticatedUser()
  if (!user) redirect(`/login?next=/app/${orgSlug}/school-pickup/${siteSlug}/students/${studentId}`)
  ...
  return (
    <SchoolPickupLayout user={user} ctx={ctx} sites={sites}>
      <StudentDetailClient ctx={ctx} studentId={studentId} classrooms={classrooms} />
    </SchoolPickupLayout>
  )
}
